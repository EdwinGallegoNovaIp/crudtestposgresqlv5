...
import com.crudtest.app.config.KafkaSseConsumer;
import com.crudtest.app.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class Crudtestposgresqlv5KafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/crudtestposgresqlv-5-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/crudtestposgresqlv-5-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/crudtestposgresqlv-5-kafka/unregister"));
                return;
            }
        }
